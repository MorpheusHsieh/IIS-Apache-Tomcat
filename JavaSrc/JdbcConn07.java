/******************************************************************************
 * JdbcConn 物件第七版和第六版最大的不同點在於對組態檔案的讀取方式改變。在設計系統      *
 * 時，我們通常會用到不只一個的資料庫，針對每個資料庫需設定不同的ODBC，在第六版中每      *
 * 個讀取資料表的類別檔需要獨立的系統組態檔，在第七版則將這些系統組態整合起來，只需要     *
 * 只需要一個系統組態檔就夠了。                                                   *
 ******************************************************************************/
...
public class JdbcConn07
{
  ...
  // 連線 ----------------------------------------------------------------------
  public void MakingConnection(String ini_file)
  {
    this.Connection(ini_file, 0);
  }

  public void MakingConnection(String ini_file, String conf_name)
  {
    boolean exist = FileExist(ini_file);

    int result = -1;
    myConfigFile mcf = new myConfigFile(ini_file);
    myConfigUnit[] mcs = mcf.ReadConfigList();
    for (int i=0; i<mcs.length; i++)
    {
      if (mcs[i].Name().equals(conf_name))
      {
        result = i;
        break;
      }
    }
    this.Connection(ini_file, result);
  }

  private void Connection(String ini_file, int conf_no)
  {
    myConfigFile mcf = new myConfigFile(ini_file);
    myConfigUnit[] mcs = mcf.ReadConfigList();
    String user = mcs[conf_no].Search("UserName");
    String pswd = mcs[conf_no].Search("Password");
    String driver = mcs[conf_no].Search("DriverName");
    String url = mcs[conf_no].Search("URL");

    try
    {
      Class.forName(driver);  // Load the driver class
      if (user != null && pswd != null) // create a connection throught the DriverManager
  ...
  private boolean FileExist(String ini_file)
  {
    boolean result = false;
    File myFile = new File(ini_file);
    result = myFile.exists();
    return result;
  }
