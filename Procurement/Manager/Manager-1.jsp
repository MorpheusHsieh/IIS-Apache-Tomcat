...
<form name="frmManager01" method="post" action="<%=strMySelf%>"> 
  ...
  <p>我是<font color="#0000FF">年度計畫管制</font>的頁面...</p>
  ...
  <div align="center">
    ...
    <%
      ...
      if ((strSend != null) && (strSend.length() > 0)) {
        nRowsCount = clsManager.QueryForPlan(strSend);
        isActiveHyperlink = false;
	  } else if (strSend == null) {
        nRowsCount = clsManager.QueryForYearlyPlan();
		  ...
		  <tr>
		    <%
		  	  for (int col=0; col<cols; col++) {
		 	    outstr += "<td align=\"center\">" + fields[col].getCaption() + "</td>\n";
			  }
			  out.print(outstr);
		    %>
		  </tr>
		  <%
		    for (int row=0 ;row<rows; row++)
		    {
		      ...
		      for (int col=0; col<cols; col++)
		      {
		        ...
		        {
		          outstr += "<a href=\"Manager-1.jsp?BtnSend=" + strValue + "\">" + strValue + "</a>";
		        } else {
	  	          outstr += strValue;
	  	        }  
		        outstr += "</td>";
	...
	%>
    <p>
      <a href="Manager-1.jsp">回「年度計畫查詢」頁面</a>
    </p>  
  </div>
